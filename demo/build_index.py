...
<p class="intro">Every route the day-to-day accountant touches, captured against the pharma sample. Click any screenshot to open the live Bahi route on localhost:8101 (load <code>pharma.khata</code> first).</p>
{accountant_html}
...
<p class="intro">Every screenshot on this page is generated by <code>capture.py</code> — a Playwright script that drives the live Bahi instance at localhost:8101, loads each sample file via Bahi's in-page globals, navigates to each route, and records a 1400×900 PNG. The script doubles as a regression test: any route that fails to render or throws a console error is flagged in <a href="CAPTURE-LOG.md">CAPTURE-LOG.md</a>.</p>
<div class="why-grid">
